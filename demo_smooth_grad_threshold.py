...
# image_path = "/root/autodl-tmp/SQA/test/85/image.png"
# image_path = "/root/autodl-tmp/OPERA/image/1.png"
image_path = "/root/autodl-tmp/OPERA/image/17.jpg"
# image_path = "/root/autodl-tmp/OPERA/1723436753400.jpg"
# image_path = "/root/autodl-tmp/OPERA/image/5.jpg"
...
# question = [{"content":"<image>\nContext: Below is a food web from Little Rock Lake, a freshwater lake ecosystem in Wisconsin. A food web models how the matter eaten by organisms moves through an ecosystem. The arrows in a food web represent how matter moves between organisms in an ecosystem. Question: Which of the following organisms is the primary consumer in this food web? Options: (A) copepod (B) black crappie (C) bacteria<image>\nAnswer with the option's letter from the given choices directly."}]
...
for i in range (0,32):
    save_path = 'NAACL/17_smooth_threshold'
    gradcam = SmoothGradCAM(model, model.llama_model.model.layers[i].post_attention_layernorm, input_token_len, output_ids)
    # gradcam = SmoothGradCAM(model, model.llama_model.model.layers[i].input_layernorm, input_token_len, output_ids)
    # gradcam = SmoothGradCAM(model, model.llama_model.model.layers[i].self_attn.v_proj, input_token_len, output_ids)
